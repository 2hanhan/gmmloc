...
namespace gmmloc
{
  ...
  void Localization::jointOptimization(KeyFrame *kf_ptr, bool *pbStopFlag, Map *pMap)
  {
    ...
    // step 1 获取所有的共视keyframe和其中的mappoint
    ...
    // - 统计每个keyframe观测的mappoint个数，提取出最佳观测的frame，即是观测mappoint最多的keyframe
    ...
    {
      std::unordered_map<KeyFrame *, size_t> observations = (*lit)->getObservations();
    ...
    if (!fixcam_obs.empty())
    {
      std::vector<std::pair<KeyFrame *, int>> v(fixcam_obs.begin(), fixcam_obs.end());
         ...
         lit != lend; lit++)
    {
      ...
      if (kfi_ptr->idx_ == 0) //第一帧
      {
        ...
        if (loc::ba_first_as_prior)
        {
          ...
          info.block(3, 3, 3, 3) = Eigen::Matrix3d::Identity() * sigma_trans2_inv; // trans
    ...
    for (list<MapPoint *>::iterator lit = local_mappts.begin(), lend = local_mappts.end(); lit != lend; lit++)
    {
      ...
      if (!mappt->asscociations_.empty())
      {
        ...
        for (size_t i = 0; i < mappt->asscociations_.size(); i++)
        {
          ...
          if (comp3d->is_degenerated)
          {
            ...
            e->setInformation(Eigen::Matrix<double, 1, 1>::Identity() * loc::ba_lambda2);
      ...
      const unordered_map<KeyFrame *, size_t> observations = mappt->getObservations();
    ...
    // step 3 遍历所有keyframe的单目+双目约束，去除keyframe中不好的mappoint
    ...
    // step 4 取出优化完成的keyframes 位姿和loacl map points位姿
