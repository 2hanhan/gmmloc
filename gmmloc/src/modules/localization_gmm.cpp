...
namespace gmmloc
{
  ...
  /**
   * @brief 添加使用数据关联结果进行先验地图优化设置
   *
   * @param kf_ptr 当前keyframe
   * @param pbStopFlag
   * @param pMap //?这个参数好像没有使用
   */
  void Localization::jointOptimization(KeyFrame *kf_ptr, bool *pbStopFlag, Map *pMap)
  {
    ...
    for (list<MapPoint *>::iterator lit = local_mappts.begin(), lend = local_mappts.end(); lit != lend; lit++)
    {
      std::unordered_map<KeyFrame *, size_t> observations =
          (*lit)->getObservations();
      for (std::unordered_map<KeyFrame *, size_t>::iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
      {
    ...
    // - 设置 Local KeyFrame的顶点
    for (list<KeyFrame *>::iterator lit = local_kfs.begin(), lend = local_kfs.end(); lit != lend; lit++)
    {
    ...
    {
      for (list<KeyFrame *>::iterator lit = fixed_kfs.begin(), lend = fixed_kfs.end(); lit != lend; lit++)
      {
        KeyFrame *kfi_ptr = *lit;
        g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
        vSE3->setEstimate(kfi_ptr->getTcw());
    ...
    // 设置地图点顶点
    const int nExpectedSize = (local_kfs.size() + fixed_kfs.size()) * local_mappts.size();
    ...
    // - 添加 loacl map points 约束
    // * 对应论文 III. METHOD C. Residual Definitions
    for (list<MapPoint *>::iterator lit = local_mappts.begin(), lend = local_mappts.end(); lit != lend; lit++)
    {
      // 添加 loacl map point 的顶点
      MapPoint *mappt = *lit;
      ...
      if (!mappt->asscociations_.empty())
      {
        if (mappt->asscociations_.size() > 1)
        {
          throw std::runtime_error("mismatch size"); //检测运行错误
        }

        // - 遍历所有与local map point 的数据关联的先验地图的GMM
        for (size_t i = 0; i < mappt->asscociations_.size(); i++)
        {
          ...
          if (comp3d->is_degenerated)
          {
            ...
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(vPoint));       //设置连接顶点
            e->setInformation(Eigen::Matrix<double, 1, 1>::Identity() * loc::ba_lambda2); //设置信息矩阵

            e->normal_ = comp3d->axis_.col(0); // ? λ1
            e->mean_ = comp3d->mean();

            optimizer.addEdge(e); //添加边
            edges_gmm_deg.push_back(e);
            vpMapPointEdgeGMMDeg.push_back(mappt); //退化情况的local map point
          }
          else
          {
            //*非退化情况对应【公式9】
            ...
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(vPoint)); //设置连接顶点
            e->setInformation(Eigen::Matrix<double, 3, 3>::Identity());             //设置信息矩阵
            e->comp_ = comp3d;

            optimizer.addEdge(e);
            vpEdgePt2Gaussian.push_back(e);
            vpMapPointEdgeGMMNondeg.push_back(mappt); //非退化情况的local map point
          }
        }
      }

      // - 遍历所有的keyframe添加GMM观测的约束
      // * 对应论文 III. METHOD D. Projection of the GMM Map
      const unordered_map<KeyFrame *, size_t> observations = mappt->getObservations(); // keyframe的观测

      for (unordered_map<KeyFrame *, size_t>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
      {
        KeyFrame *kfi_ptr = mit->first;
        if (flag_fixsingle && kfi_ptr->fixed_kf_idx_ba == kf_ptr->idx_ && kfi_ptr != best_obs)
        {
          continue;
        }
        ...
        if (!kfi_ptr->not_valid_)
        {
          ...
          if (kfi_ptr->features_[mit->second].u_right < 0)
          {
            ...
            e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));            //设置连接顶点
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(kfi_ptr->idx_))); //设置连接顶点
            ...
            optimizer.addEdge(e); //添加边
            //保存单目观测边、frame、point，就是约束项的两个顶点和边
          ...
          else // Stereo observation
          {
            ...
            e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(kfi_ptr->idx_)));
            e->setMeasurement(obs);
            const float &invSigma2 = frame::sigma2_inv[kpUn.octave];
            ...
            optimizer.addEdge(e); //添加边
            //保存双目观测边、frame、point，就是约束项的两个顶点和边
    ...
    // - 初步优化之后将valid的约束的核函数取消，然后重新优化
    // - 遍历所有的GMM结构约束退化情况，计算误差
    ...
    // step 遍历所有keyframe的单目+双目约束，去除keyframe中不好的mappoint
    ...
    // step 取出优化完成的keyframes 位姿和loacl map points位姿
