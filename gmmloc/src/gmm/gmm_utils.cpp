...
namespace gmmloc
{
   ...
   * @param g 先验GMM
   * @param cam 相机模型
   * @param rot_c_w world在camera下的四元数
   * @param t_c_w world在camera下的位姿
   * @return GaussianComponent2d::Ptr
   */
  GaussianComponent2d::Ptr GMMUtility::projectGaussian(const GaussianComponent &g, PinholeCamera::Ptr cam,
                                                       const Eigen::Quaterniond &rot_c_w,
                                                       const Eigen::Vector3d &t_c_w)
  {
    auto &&cov3d = g.cov(); // GMM协方差
    auto &&mu3d = g.mean(); // GMM均值
    ...
    auto cov2d = jacob_proj * rot * cov3d * rot.transpose() * jacob_proj.transpose(); // GMM投影到2D的协方差

    return GaussianComponent2d::Ptr(new GaussianComponent2d(mu2d, cov2d)); //返回GMM在keyframe像素坐标系下的均值和方差
  }
